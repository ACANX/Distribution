...
# -*- coding: utf-8 -*-
"""
ArchiveJsonlTransfer.py —— SecuQuoteExecLog 归档 JSONL 跨仓库转存
================================================================================
...
约定(重要):
    ...
    * GIT_COMMIT_TOKEN 需同时具备两个仓库的权限: 读本仓库(checkout 检出用),
      写 acdnx/Distribution 的 quote 分支(Contents API 提交用)。
...
--------
    ...
    转存端仓库的 quote 分支(不依赖本地 git 提交, 因此不会与本仓库其它定时任务的
    数据推送产生 push 竞争)。
...
------------------------
    ...
    3. 脚本内置默认(acdnx / Distribution / quote)

用法
----
    python3 .github/Python/Quote/ArchiveJsonlTransfer.py --dry-run
    python3 .github/Python/Quote/ArchiveJsonlTransfer.py
    python3 .github/Python/Quote/ArchiveJsonlTransfer.py --date 20260915
