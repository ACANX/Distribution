...
# -*- coding: utf-8 -*-
"""
Task05 —— SecuQuoteExecLog 归档 JSONL 跨仓库转存
================================================================================

把**本仓库**(ACANX/Distribution, 采集归档端) quote 分支
Archive/Finv/SecuQuoteExecLog/ 下已归档的 .jsonl, 经 GitHub Contents API 转存到
**另一个仓库** acdnx/Distribution(长期归档端)的 quote 分支**同路径**下:

    本仓库  Archive/Finv/SecuQuoteExecLog/LOG_Finv_SecuQuoteExecLog_DAY_Lambda_{yyyyMMdd}.jsonl
    转存端  Archive/Finv/SecuQuoteExecLog/LOG_Finv_SecuQuoteExecLog_DAY_Lambda_{yyyyMMdd}.jsonl

    文件名与目录结构原样保留, 转存端落点 = 源路径。

    源侧由 .github/Python/Quote/Task04ArchiveSecuQuoteExecLogDaily.py 每日生成
    (文件名前缀常量在同文件的 ARCHIVE_PREFIX), 本脚本按同一命名接收 —— 两边改名
    须同步。
...
----
    ...
    注意: Task04 每天(BJT 23:55 前后)把当天数据并入**当天**那个文件, 因此当天
    的文件在当日仍会变化; 本脚本排在次日凌晨之后跑, 取到的当天文件已定稿。若源
    文件被人工修正, 转存端会跟着更新 —— 这是期望行为。

源端清理
--------
    转存**确认送达**后(本次提交成功, 或转存端已有同一份内容而跳过提交), 该源文件
    随即经 Contents API 从**源端**删除, 源端归档目录不再随日期无限增长; 提交失败的
    源文件一律保留, 留待下次重跑。
...
用法
----
    python3 .github/Python/Quote/Task05ArchiveSecuQuoteExecLogJsonlTransfer.py --dry-run
    python3 .github/Python/Quote/Task05ArchiveSecuQuoteExecLogJsonlTransfer.py
    python3 .github/Python/Quote/Task05ArchiveSecuQuoteExecLogJsonlTransfer.py --date 20260915
...
ARCHIVE_ROOT = Path("Archive") / "Finv" / "SecuQuoteExecLog"
...
def collect_files(src_root: Path, only_date=None):
    ...
    文件名末尾应为 _yyyyMMdd.jsonl(Lambda 前缀不影响); 解析不出日期的文件跳过并
    告警(宁可不搬也不误搬)。返回按 (日期, 路径) 升序的
    [(绝对路径, 仓库内相对路径, 日期)]。
    """
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="%s 归档 JSONL 跨仓库转存到 %s/%s, 并删除源端已送达的文件(Contents API)"
    ...
    # 不再随日期无限增长。只删已送达的; 提交失败的源文件保留, 留待下次重跑。
    deleted, gone = [], []
