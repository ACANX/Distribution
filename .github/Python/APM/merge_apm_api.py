...
# -*- coding: utf-8 -*-
"""
merge_apm_api —— API 埋点 Data 侧批次按日聚合为 JSONL(Contents API 提交)
================================================================================

历史版本说明
------------
    本脚本原先位于 apm 分支顶层 Python/ 目录, 把 Data/Meta/WebMMCP/APM/API/
    下全部批次 json 聚合为一个 Archive/Meta/WebMMCP/APM/API/{时间戳}.json
...
-----------------
    ...
    2. 职责升级: 由"全量聚合为时间戳 .json"改为"按日聚合为 JSONL" —— 只聚合
       **指定某一天**(缺省为运行时刻的北京时间日历日; --date YYYYMMDD 可覆盖,
       可重复传多日) Data 侧的批次, 输出
           Archive/Meta/WebMMCP/APM/API/APM_API_MetaCmsAPI_DAY_ACANX_{yyyyMMdd}.jsonl
...
用法
----
    python3 .github/Python/APM/merge_apm_api.py                # 聚合"今天"
    python3 .github/Python/APM/merge_apm_api.py --date 20260629
    python3 .github/Python/APM/merge_apm_api.py --dry-run

    本文件是 .github/Python/APM/APMJsonlConvert.DailyJsonl.py 的薄封装
    (限定 --type API 且只扫 Data 侧), 分组/合并/提交逻辑全部在前者维护。
...
# ── 路径(与原 merge_apm_api.py 的 DATA_DIR 一致) ─────────────────────────────
