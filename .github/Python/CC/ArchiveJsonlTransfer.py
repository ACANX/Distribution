...
# -*- coding: utf-8 -*-
"""
ArchiveJsonlTransfer.py —— CC/SSE 归档 JSONL 跨仓库转存
================================================================================

把**本仓库**(ACANX/Distribution, 采集归档端) cc-sse 分支
Archive/CC/SSE/ 下已归档的 .jsonl, 经 GitHub Contents API 转存到**另一个仓库**
acdnx/Distribution(长期归档端)的 cc-sse 分支**同路径**下:

    本仓库  Archive/CC/SSE/CC_SSE_CCSSEStreamReqResp_{yyyyMMdd}.jsonl
    转存端  Archive/CC/SSE/CC_SSE_CCSSEStreamReqResp_{yyyyMMdd}.jsonl
...
约定(重要):
    ...
    * GIT_COMMIT_TOKEN 需同时具备两个仓库的权限: 读本仓库(checkout 检出用),
      写 acdnx/Distribution 的 cc-sse 分支(Contents API 提交用)。
...
----
    ...
    注意: 当天的归档文件在本仓库会被聚合工作流继续追加(当天那份还在增长), 此时
    转存端会跟着更新 —— 这是期望行为, 转存端永远与源端最终版一致。

提交方式
--------
    复用同仓 .github/Python/GitHubCommitContent.py, 经 GitHub Contents API 提交到
    转存端仓库的 cc-sse 分支(不依赖本地 git 提交, 因此不会与本仓库其它定时任务的
    数据推送产生 push 竞争)。
...
------------------------
    ...
    3. 脚本内置默认(acdnx / Distribution / cc-sse)

用法
----
    python3 .github/Python/CC/ArchiveJsonlTransfer.py --dry-run
    python3 .github/Python/CC/ArchiveJsonlTransfer.py
    python3 .github/Python/CC/ArchiveJsonlTransfer.py --date 20260618
...
TARGET_BRANCH = os.environ.get("ARCHIVE_TRANSFER_BRANCH", "cc-sse")
TOKEN_ENV = "GIT_COMMIT_TOKEN"

# 源目录(仓库内相对路径); 文件名末尾的 yyyyMMdd 作为日期
ARCHIVE_ROOT = Path("Archive") / "CC" / "SSE"
...
def repo_root() -> Path:
    """仓库根目录。本文件位于 <根>/.github/Python/CC/, 上溯三级即根。"""
