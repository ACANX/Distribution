...
$env:GITHUB_COMMIT_TOKEN = "ghp_你的Token"；严禁把令牌写进任何源码）。
...
|             | 2) 环境变量 GITHUB_COMMIT_TOKEN（约定专用变量名）        |      |
...
----------------------------------------------------------------------------------------
    ...
    - 运行/调试时禁止把 "$env:GITHUB_COMMIT_TOKEN" 的内容打印到 stdout/stderr；
...
def commit_content(path_key, content, branch=None, commit_msg=None,
    ...
    行为要点：先 GET 查 sha（404 视为新建）再 PUT；默认分支 "latest"、默认提交信息
    "UpdatedAt@<当前本地时间>"、author/committer 固定为通用 bot 身份 github-bot；
    失败不抛异常，一律以返回 dict 表示。
    ...
    # 3) 默认值：branch→"latest"，commit_msg→"UpdatedAt@"+当前本地时间
...
def commit_content_file(path_key, local_file, branch=None, commit_msg=None,
    ...
    :param token: 访问令牌；None = 环境变量 GITHUB_COMMIT_TOKEN
    :param api_base: GitHub API 仓库根；默认 https://api.github.com/repos
    :param timeout: 单次 HTTP 请求超时秒数，默认 30
