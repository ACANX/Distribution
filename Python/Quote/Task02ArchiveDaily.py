...
def process_code(code, config, log):
    ...
    # ── 1. 归档超过 DailyArchiveAfterDays 个自然日的数据（复制导出，不从 Latest 移除） ──
    now_bjt = datetime.now(BJT)
    archive_cutoff_date = now_bjt.date() - timedelta(days=config.daily_archive_after_days)
    cutoff = int(datetime.combine(archive_cutoff_date, time(0, 0, 0), tzinfo=BJT).astimezone(UTC).timestamp())
    archive_candidates = [r for r in latest.rows if int(r[0]) < cutoff]
    archived_dates = []
    if archive_candidates:
        log.info(f'待归档: {len(archive_candidates)} 行（cutoff={archive_cutoff_date.strftime("%Y%m%d")} BJT 00:00）')
        by_date = {}
    ...
    # ── 2. 裁剪超过 LatestWindowDays 个自然日的数据 ──
    before_trim = len(latest.rows)
    trim_cutoff_date = now_bjt.date() - timedelta(days=config.latest_window_days)
    cutoff_latest = int(datetime.combine(trim_cutoff_date, time(0, 0, 0), tzinfo=BJT).astimezone(UTC).timestamp())
    latest.rows = [r for r in latest.rows if int(r[0]) >= cutoff_latest]
    trimmed = before_trim - len(latest.rows)
    if trimmed:
        log.info(f'Latest 裁剪: {trimmed} 行（cutoff={trim_cutoff_date.strftime("%Y%m%d")} BJT 00:00，超过 {config.latest_window_days} 天）')
    else:
    ...
    if trimmed:
        ...
        sha2 = gitutil.commit(
            f'[quote] trim Latest for {code} (trim {trimmed} rows)',
            cwd=str(config.repo_root),
        )
        if sha2:
            log.info(f'裁剪 commit: {sha2}')

    # ── 4. 有任何变更时统一 push（归档 commit + 裁剪 commit 一起推） ──
    if archived_dates or trimmed:
